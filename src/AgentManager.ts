...
export class AgentManager {
    ...
    async exportAgent(name: string, scope: 'local' | 'user'): Promise<string> {
        const agent = await this.getAgent(name, scope);

        if (!agent) {
            throw new Error(`Agent ${name} not found in ${scope} scope`);
        }

        return this.formatAgentFile(agent);
    }

    async importAgent(content: string, scope: 'local' | 'user', overwrite: boolean = false): Promise<Agent> {
        const { metadata, systemPrompt } = this.parseAgentFile(content);
        ...
        return await this.createAgent(agent, overwrite);
    }
}